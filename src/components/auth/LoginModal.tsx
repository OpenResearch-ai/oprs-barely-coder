...
export default function LoginModal({ onClose, redirectTo }: Props) {
  ...
  const handleEmail = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim() || !password.trim()) { setError(t("error_fields")); return; }
    if (password.length < 6) { setError(t("error_password_short")); return; }
    ...
    if (isSignup) {
      ...
      if (error) setError(error.message === "User already registered" ? t("error_already_registered") : error.message);
      else setSuccess(t("success_signup"));
    } else {
      const { error } = await supabase.auth.signInWithPassword({
        email: email.trim(),
        password: password.trim(),
      });
      if (error) setError(t("error_invalid_credentials"));
      else onClose();
    }
    setLoading(false);
  };
  ...
  const sendReset = async (e: React.FormEvent) => {
    ...
    else setSuccess(t("success_reset"));
    setLoading(false);
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-sm bg-white rounded-3xl overflow-hidden animate-slide-up"
        ...
        <div className="px-7 pt-7 pb-4 text-center">
          ...
          <h2 className="text-base font-bold mb-0.5">{t("community")}</h2>
        ...
        <div className="px-7 pb-7">
          ...
          {view === "main" && (
            <div className="space-y-2.5">
              <button onClick={signInWithGoogle} disabled={googleLoading}
                ...
                <span className="flex-1 text-left">{t("google")}</span>
              </button>
              ...
              <button onClick={() => { setView("email"); reset(); }}
                ...
                <span className="flex-1 text-left">{t("email")}</span>
              </button>

              <p className="text-center text-[10px] text-[var(--text-tertiary)] pt-1">{t("terms")}</p>
            </div>
          )}
            ...
            <div>
              ...
              {success ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-sm font-bold mb-1">{t("check_email_title")}</p>
                ...
                <form onSubmit={handleEmail} className="space-y-3">
                  ...
                  <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                    placeholder={isSignup ? t("password_signup_placeholder") : t("password_placeholder")} required minLength={6}
                    className="w-full px-4 py-3 text-sm rounded-2xl border border-[var(--border)] outline-none focus:border-[var(--purple-muted)] transition-all" />
                  ...
                  <button type="submit" disabled={loading}
                    ...
                    {loading ? t("processing") : isSignup ? t("signup") : t("login_btn")}
                  </button>
                  {!isSignup && (
          ...
          {view === "forgot" && (
            <div>
              <BackBtn to="email" />
              <h3 className="text-sm font-bold mb-1">{t("reset_title")}</h3>
              <p className="text-xs text-[var(--text-secondary)] mb-4">{t("reset_desc")}</p>
              {success ? (
